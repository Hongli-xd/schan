...
class FunAsrProvider implements AsrProvider {
  ...
  async transcribe(audio: AsyncIterable<Buffer>): Promise<string> {
    ...
    try {
      ...
      if (existsSync(tmpWav16k)) {
        try { unlinkSync(tmpWav16k); } catch { /* ignore */ }
      }
    }
  }

  private detectWavSampleRate(buffer: Buffer): number {
  ...
  private resampleWav(inputPath: string, outputPath: string, fromRate: number, toRate: number): void {
...
`;
  ...
  private callFunAsr(wavPath: string): Promise<string> {
