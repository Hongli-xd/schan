...
import { StackChanMsgType, type StackChanMessage } from "../protocol.js";

/**
 * Create a StackChan binary message
 */
export function createStackChanMessage(msgType: number, payload?: Buffer): Buffer {
 ...
 */
export function createStackChanRoutedMessage(
  msgType: number,
  mac: string,
  data: Buffer,
): Buffer {
 ...
 */
export function parseStackChanMessage(data: Buffer): StackChanMessage | null {
  if (data.length < 5) {
    return null;
  }
...
export class SimulatedStackChan {
  private messageHandlers: Array<(msg: StackChanMessage) => void> = [];
  private mac: string;
  private name: string;

  constructor(mac: string = "AABBCCDDEEFF", name: string = "SimulatedStackChan") {
    this.mac = mac;
    this.name = name;
  }
  ...
  onMessage(handler: (msg: StackChanMessage) => void): void {
   ...
   */
  sendAudio(audioData: Buffer): Buffer {
...
export class XiaozhiToStackChanTranslator {
   ...
   */
  static fromXiaozhiJson(type: string, data: unknown, robotMac: string): Buffer | null {
    switch (type) {
      case "tts": {
        const ttsData = data as { text?: string; state?: string };
        if (ttsData.text) {
      ...
      case "mcp": {
        const mcpData = data as { method?: string; arguments?: Record<string, unknown> };
        if (mcpData.method?.startsWith("self.robot.set_led_color")) {
          const { red = 0, green = 0, blue = 0 } = mcpData.arguments ?? {};
          const controlData = Buffer.from([red as number, green as number, blue as number]);
        ...
        if (mcpData.method?.startsWith("self.robot.set_head_angles")) {
          ...
          controlData.writeFloatBE(pitch as number, 0);
          controlData.writeFloatBE(yaw as number, 4);
          controlData.writeFloatBE(speed as number, 8);
...
export class StackChanToXiaozhiTranslator {
   ...
   */
  static toXiaozhiJson(msg: StackChanMessage): object | null {
    switch (msg.msgType) {
      case StackChanMsgType.TextMessage: {
        const text = msg.payload.toString("utf8");
...
export async function runSimulation(): Promise<void> {
  ...
  const ttsBinary = XiaozhiToStackChanTranslator.fromXiaozhiJson(
    "tts",
  ...
  const ledBinary = XiaozhiToStackChanTranslator.fromXiaozhiJson(
    "mcp",
    { method: "self.robot.set_led_color", arguments: { red: 255, green: 0, blue: 128 } },
    robot.getMac(),
  );
  ...
  const motionBinary = XiaozhiToStackChanTranslator.fromXiaozhiJson(
    "mcp",
    { method: "self.robot.set_head_angles", arguments: { pitch: 20, yaw: -15, speed: 180 } },
    robot.getMac(),
  );
