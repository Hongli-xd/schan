...
class SimulatedStackChanFirmware {
  ...
  private handleTextMessage(raw: string): void {
    try {
      ...
      switch (msg.type) {
        case "hello":
          this.handleServerHello(msg as ServerHello);
          break;
   ...
   */
  private handleServerHello(msg: ServerHello): void {
   ...
   */
  private handleTts(msg: { state: string; text?: string }): void {
    switch (msg.state) {
      case "start":
   ...
   */
  private handleStt(msg: { text: string }): void {
   ...
   */
  private handleLlm(msg: { emotion?: string; text?: string }): void {
   ...
   */
  private handleMcp(msg: { payload: McpJsonRpc; session_id?: string }): void {
    const { payload } = msg;
   ...
   */
  private handleMcpRequest(payload: McpJsonRpc): void {
    ...
    switch (method) {
      ...
      case "tools/call": {
        const params = payload.params as { name?: string; arguments?: Record<string, unknown> } | undefined;
   ...
   */
  private simulateRobotAction(name: string, args: Record<string, unknown>): void {
    if (name === "self.robot.set_led_color") {
      ...
      const { pitch, yaw, speed } = args as { pitch?: number; yaw?: number; speed?: number };
   ...
   * Send MCP response
   */
  private sendMcpResponse(response: McpJsonRpc): void {
    this.sendJson({
      type: "mcp",
   ...
   */
  private async runInteractionDemo(): Promise<void> {
   ...
   */
  private generateFakeOpusFrame(): Buffer {
