...
import {
  SimulatedStackChan,
  createStackChanMessage,
  createStackChanRoutedMessage,
  parseStackChanMessage,
  StackChanMsgType,
  XiaozhiToStackChanTranslator,
  StackChanToXiaozhiTranslator,
} from "./stackchan-sim.js";
...
describe("XiaozhiToStackChanTranslator", () => {
  it("should convert xiaozhi TTS to StackChan TextMessage", () => {
    ...
    const binary = XiaozhiToStackChanTranslator.fromXiaozhiJson(
      "tts",
      { text: "Hello world" },
      robotMac,
    );
  ...
  it("should convert LED color MCP call to ControlAvatar", () => {
    ...
    const binary = XiaozhiToStackChanTranslator.fromXiaozhiJson(
      "mcp",
      { method: "self.robot.set_led_color", arguments: { red: 255, green: 128, blue: 64 } },
      robotMac,
    );
