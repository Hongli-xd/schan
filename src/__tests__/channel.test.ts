...
describe("xiaozhiPlugin", () => {
  ...
  it("has default account configuration", () => {
    const accountIds = xiaozhiPlugin.config.listAccountIds({} as never);
    expect(accountIds).toEqual(["default"]);

    const account = xiaozhiPlugin.config.resolveAccount({} as never, "default");
    ...
    const defaultAccountId = xiaozhiPlugin.config.defaultAccountId({} as never);
