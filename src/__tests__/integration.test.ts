...
import type { ReplyHandler, TtsProvider, AsrProvider } from "../session.js";
import type { ChannelGatewayContext } from "openclaw/plugin-sdk";
import type { Logger } from "openclaw/plugin-sdk";

// Mock ReplyHandler
class MockReplyHandler implements ReplyHandler {
  calls: Array<{ text: string; mcp: McpManager }> = [];
  ...
  async reply(
    ...
    ws: WebSocketLike,
    log: Logger,
    ctx: ChannelGatewayContext<{ accountId: string }>,
  ): Promise<void> {
...
class MockAsrProvider implements AsrProvider {
  constructor(private result = "今天天气怎么样？") {}
...
function createMockCtx(): ChannelGatewayContext<{ accountId: string }> {
  return {
    accountId: "default",
    cfg: {},
    channelRuntime: {
    ...
    } as unknown as ChannelGatewayContext<{ accountId: string }>["channelRuntime"],
  } as ChannelGatewayContext<{ accountId: string }>;
}
...
describe("xiaozhi Plugin Integration", () => {
  describe("Session handshake", () => {
    it("completes hello handshake", async () => {
      ...
      const session = new XiaozhiSession(
        ...
        ws,
        { port: 8765 },
        new McpManager("test-conn-1", ws, mockLogger),
        asr,
        tts,
        replyHandler,
        ctx,
  ...
  describe("MCP tools handling", () => {
    ...
    it("handles tools/call request", async () => {
      ...
      const onMcpCallLogged = mockLogger.info?.mock.calls.some((c) =>
        c[0]?.includes?.("Device calling tool"),
      );
...
describe("Action parsing in replies", () => {
  function parseActionJson(text: string): { action: { act?: string; emo?: string; spd?: number } | null; remainingText: string } {
